...
func NewDHTNodeWithProxyPubsub(logger *zap.Logger, configConfig *config.Config, uint2 uint) (*DHTNode, error) {
	p2PConfig := configConfig.P2P
	engineConfig := configConfig.Engine
	proxyBlossomSub, err := rpc.NewProxyBlossomSub(p2PConfig, engineConfig, logger, uint2)
	if err != nil {
		return nil, err
	}
	dhtNode, err := newDHTNode(proxyBlossomSub)
	if err != nil {
		return nil, err
	}
	return dhtNode, nil
...
func NewDHTNodeWithoutProxyPubsub(logger *zap.Logger, configConfig *config.Config, uint2 uint) (*DHTNode, error) {
	p2PConfig := configConfig.P2P
	engineConfig := configConfig.Engine
...
func NewMasterNodeWithProxyPubsub(logger *zap.Logger, config2 *config.Config, coreId uint) (*MasterNode, error) {
	dbConfig := config2.DB
	pebbleDB := store2.NewPebbleDB(logger, dbConfig, coreId)
	pebbleDataProofStore := store2.NewPebbleDataProofStore(pebbleDB, logger)
	pebbleClockStore := store2.NewPebbleClockStore(pebbleDB, logger)
	...
	doubleRatchetEncryptedChannel := channel.NewDoubleRatchetEncryptedChannel()
	bedlamCompiler := compiler.NewBedlamCompiler()
	consensusEngineFactory := global.NewConsensusEngineFactory(logger, config2, proxyBlossomSub, hypergraph, fileKeyManager, pebbleKeyStore, frameProver, kzgInclusionProver, cachedSignerRegistry, proverRegistry, dynamicFeeManager, blsAppFrameValidator, blsGlobalFrameValidator, asertDifficultyAdjuster, optimizedProofOfMeaningfulWorkRewardIssuance, pebbleClockStore, pebbleInboxStore, pebbleHypergraphStore, pebbleShardsStore, pebbleWorkerStore, pebbleConsensusStore, doubleRatchetEncryptedChannel, decaf448BulletproofProver, mpCitHVerifiableEncryptor, decaf448KeyConstructor, bedlamCompiler, bls48581KeyConstructor, inMemoryPeerInfoManager)
	...
	masterNode, err := newMasterNode(logger, pebbleDataProofStore, pebbleClockStore, pebbleTokenStore, fileKeyManager, proxyBlossomSub, inMemoryPeerInfoManager, globalConsensusEngine, globalTimeReel, pebbleDB, coreId)
	if err != nil {
		return nil, err
	}
...
func NewMasterNodeWithoutProxyPubsub(logger *zap.Logger, config2 *config.Config, coreId uint) (*MasterNode, error) {
	dbConfig := config2.DB
	pebbleDB := store2.NewPebbleDB(logger, dbConfig, coreId)
	pebbleDataProofStore := store2.NewPebbleDataProofStore(pebbleDB, logger)
	pebbleClockStore := store2.NewPebbleClockStore(pebbleDB, logger)
...
func NewDHTNode(logger *zap.Logger, config2 *config.Config, coreId uint) (*DHTNode, error) {
	if config2.P2P.UseProxyBlossomSub {
		return NewDHTNodeWithProxyPubsub(logger, config2, coreId)
	} else {
		return NewDHTNodeWithoutProxyPubsub(logger, config2, coreId)
	}
}
...
func NewMasterNode(
	...
	if config2.P2P.UseProxyBlossomSub {
		return NewMasterNodeWithProxyPubsub(logger, config2, coreId)
	} else {
		return NewMasterNodeWithoutProxyPubsub(logger, config2, coreId)
	}
}
