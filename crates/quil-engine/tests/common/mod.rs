...
impl AppShardHarness {
    ...
    /// decode and return that worker's first `count` frames. Keeping all frames
    /// from one worker avoids counting the same committee finalization once per
    /// replica when a test checks frame-number uniqueness.
        ...
        while std::time::Instant::now() < deadline {
            for worker in &self.workers {
                ...
                let decoded = captured
                    .iter()
                    .map(|bytes| {
                        <gpb::AppShardFrame as prost::Message>::decode(bytes.as_slice()).ok()
                    })
